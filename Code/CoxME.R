...
## build dataset
data <- subset(new,hosp>=1,select = c(COD_REG,hosp,event,timeEvent,SESSO,ADERENTE,etaEvent,comorbidity,cens))
data$hosp<-data$hosp-1
names(data)
# add censoring event per patient
codici<- unique(data$COD_REG)
for(i in 1:length(codici)){
  paz_corrente <- codici[i]
  temp <- data.frame(paz_corrente,NA,0,unique(new[COD_REG==paz_corrente]$timeOUT),unique(new[COD_REG==paz_corrente]$SESSO), 
...
#codici<-codici[1:2500]
#data<-data[COD_REG %in% codici]

# Define dataset for terminal events
...
g1<-ggplot(data=df_cox_rec[1,]) +
  ...
  scale_x_continuous(name="",breaks = c(0.0),labels=c("Sex [F]"), limits = c(-0.1,0.1))+
  ylab("")
g2<-ggplot(data=df_cox_rec[2,]) +
  ...
  theme(legend.position="none")+
  scale_x_continuous(name="",breaks = c(0.1666667),labels=c("Adherence [0]"), limits = c(0.06666667,0.26666667))+
  ylab("")
