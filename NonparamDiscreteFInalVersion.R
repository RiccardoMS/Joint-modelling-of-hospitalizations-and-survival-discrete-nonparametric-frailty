...
if(Uniform){
  lab<-c("P1: 0.21","P2: 0.23","P3: 0.21","P4: 0.22","P5: 0.07","P6: 0.06")
}else{
  lab<-c("P1: 0.50","P2: 0.21","P3: 0.11","P4: 0.12","P5: 0.01","P6: 0.04")
}
...
plot1<-FinalResults %>%
  ...
  annotate("label", x = -0.4, y = -1.7, size=2.5,label = "P1 - Protected") +
  annotate("label", x = 0.25, y = -0.4, size=2.5,label = "P2 - Neutral") +
  annotate("label", x = 0.1, y = 1.65, size=2.5,label = "P3 - Slightly at Risk") +
  annotate("label", x = 0.5, y = 1.9, size=2.5,label = "P4 - At Risk") +
  annotate("label", x = 0.42, y = 3.7, size=2.5,label = "P6 - Outliers H") +
  annotate("label", x = 0.95, y = 2.3, size=2.5,label = "P5 - Outliers D") +
  theme(axis.title = element_text())+
  xlim(-1.0,1.0)+
  ylim(-2,4)+
  theme_fivethirtyeight() +
  ...
  plot1<-FinalResults %>%
    ...
    geom_point(alpha = 1, stroke = 0, aes(size=w,
                                          fill=factor(score,
                                                      labels = lab)),colour="black",pch=21) +
    #geom_text(hjust=2,vjust=2.5,aes(size=0.01,label=Point))+
    annotate("label", x = -0.65, y = -1.7, size=2.5,label = "P1 - Highly Protected") +
    annotate("label", x = -0.05, y = -1.1, size=2.5,label = "P2 - Protected") +
    annotate("label", x = -0.05, y = 0.2, size=2.5,label = "P3 - Neutral") +
    annotate("label", x = 0.35, y = 0.8, size=2.5,label = "P4 - At Risk") +
    annotate("label", x = 0.3, y = 2.4, size=2.5,label = "P5 - Fragile") +
    annotate("label", x = 0.75, y = 2.6, size=2.5,label = "P6 - Very Fragile") +
    theme(axis.title = element_text())+
    xlim(-1.0,1.0)+
    ylim(-2.25,4)+
    theme_fivethirtyeight() +
...
g1<-ggplot(data=temp, aes(x=time, y=exp(-value)))+
  ...
  labs(title = "Stratified Baselines for hospitalization",
       ...
       caption = "Drug: ACE Inhibitors \nInitialization: Uniform \nMinDist: 0.25",
       colour="Latent Class")+
  scale_color_manual(values = c("#FFFFFF",cc), labels=c("No random effects","P1 - Highly Protected",
                                                        "P2 - Protected",
                                                        "P3 - Neutral",
                                                        "P4 - At Risk",
                                                        "P6 - Outliers H",
  ...
  g1<-ggplot(data=temp, aes(x=time, y=exp(-value)))+
    ...
    labs(title = "Stratified Baselines for hospitalization",
         ...
         caption = "Drug: ACE Inhibitors \nInitialization: Uniform \nMinDist: 0.25",
         colour="Latent Class")+
    scale_color_manual(values = c("#FFFFFF",cc), labels=c("No random effects","P1 - Protected",
                                                          "P2 - Neutral",
                                                          "P3 - Slightly at Risk",
                                                          "P4 - At Risk",
                                                          "P5 - Fragile",
